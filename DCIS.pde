...
rand.setO2(pow(rand.r(),4-rand.aggression)/pow(crowdFactor(rand),2));
  float prob=rand.o2()/(.001*rand.elapsed());
float base=pow(r,4-rand.aggression)/pow(crowdInitial,2);
float probsimp=(5+(0.5*rand.aggression))*prob/base;
...
if(!dead&&rand.elapsed()>rand.cycleLength()){
  ...
  if(random(1)<probsimp*15/crowdFactor(rand)){
   divide(rand);
...
//  rand.add(PVector.random2D().mult(2));
  ...
  for(Cell c: points){
fill(c.aggression*60, 200, 500-20*c.aggression);
